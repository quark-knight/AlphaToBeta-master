...
def reward_function_with_env_counts(protein_sequence:                               str,
    ...
    # 4) If sheet dominates, fill the 20-aa frequency array from neighborhood
        ...
        if sheet_pct > helix_pct:
            aa_counts = count_aa_within_cutoff(path_to_the_predicted_sturcture_file,
                                               ...
                                               chain_id=chain_id,
                                               exclude_segment=True)
    # 5) If aa_counts is filled, update cumulative histogram and save to CSV
        if aa_counts is not None:
            ...
            assert HISTOGRAM_CSV_PATH is not None # 
            with open(HISTOGRAM_CSV_PATH, mode='a', newline='') as csvfile:
                    writer = csv.writer(csvfile)
    ...
    # 6) Cleanup policy: if not validation, always remove; if validation, remove only "bad" structures
        if not validation:
